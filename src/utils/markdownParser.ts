...
// Simple and reliable frontmatter parsing function
function parseFrontmatter(content: string): { frontmatter: any; content: string } {
  ...
  console.log('Frontmatter section:', frontmatterStr);
  console.log('Body content start:', bodyContent.substring(0, 100) + '...');
  
  try {
    // Simple line-by-line parsing for basic YAML-like structure
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (inArray) {
        if (line === ']') {
          ...
          arrayItems.push(line.substring(1).trim().replace(/['"]/g, '').replace(/,$/, ''));
        } else if (line.includes(',')) {
          // Handle inline array items
          arrayItems.push(line.replace(/['"]/g, '').replace(/,$/, ''));
        }
        continue;
      }
      ...
      if (inObject) {
        ...
        if (bracketCount <= 0) {
          ...
          try {
            ...
            const objStr = objectContent.replace(/[{}]/g, '').trim();
            const obj: any = {};
            ...
            console.warn('Failed to parse object:', objectContent);
      ...
      if (colonIndex > -1) {
        ...
        if (value === '[') {
          ...
          inObject = true;
          objectContent = value + ' ';
          bracketCount = (value.match(/{/g) || []).length - (value.match(/}/g) || []).length;
          
          // Handle inline objects
          if (bracketCount <= 0) {
            try {
              const objStr = value.replace(/[{}]/g, '').trim();
              const obj: any = {};
              ...
              if (objStr) {
                ...
                for (const pair of pairs) {
                  ...
                  if (pairColonIndex > -1) {
                    const objKey = pair.substring(0, pairColonIndex).trim().replace(/['"]/g, '');
                    const objValue = pair.substring(pairColonIndex + 1).trim().replace(/['"]/g, '');
                    if (objKey && objValue) {
                      obj[objKey] = objValue;
                    }
                  }
